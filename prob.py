...
# TODO WARNING предупреждать, что ratio метрики как continuous метрики

st.title("Sample size calculator")

metric_type = st.selectbox("Select metric type:", ("continuous", "binary"))
st.write("You choose:", metric_type)

if metric_type == "continuous":
    is_skewed = st.checkbox("Data is skewed")
    st.write(is_skewed)
    mde = st.slider("MDE", min_value=0.01, max_value=0.99, value=0.03, step=0.01)
    power = st.number_input(
        "Power", min_value=0.00, max_value=0.99, value=0.80, format="%.2f"
    )

    alpha = st.number_input(
        "alpha", min_value=0.0, max_value=0.99, value=0.05, format="%.5f"
    )

    alternative = st.selectbox(
        "Select alternative hypothesis:", ("two-sided", "larger", "smaller")
    )
